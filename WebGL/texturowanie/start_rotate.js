...
//************************************************************************************
	...
	gl.uniform1i(gl.getUniformLocation(program, "texture1"), 0); // set it manually
	gl.uniform1i(gl.getUniformLocation(program, "texture2"), 1); // set it manually
...
let kat_chwilowy=0;
  ...
  //              gl.drawArrays(prymityw, 0, n_draw);
				...
				switch(tryb)
				{
					case 0:
						...
						StereoProjection(-6, 6, -4.8, 4.8, 12.99,  -100, 0,  13,-separation);
						...
						StereoProjection(-6, 6, -4.8, 4.8, 12.99, -100, 13,separation);
   ...
   kat_chwilowy+=0.5;
  const model = mat4.create();
  let kat_obrotu = kat_chwilowy * Math.PI / 180;   // in radians
  ...
  mat4.translate(model, model, [0, 0, -2]);  
	...
	//load more than 1 texture in the same time
		...
		mat4.translate(model, model, [-2, -2, 0]);  
		gl.uniformMatrix4fv( uniModel, false, model);
         gl.drawArrays(prymityw, 0, 36);
